...
def validDict(dict):
    ...
    if((-128 < dict["motor_speed"] < 128) and ((dict["motor_direction"]  and dict["steering_direction"])in range(0,2)) and -128 < dict["steering_position"] < 128):
        ...
        b = False
    return b
#del everything below this


def test():
    flags = Flags_motor()
    flags.asByte = int("11111111",2) 
    print(int("11111111",2))
    print( "direction: %i" % flags.direction     )
    print( "speed:  %i" % flags.speed )
    del flags
    flags = Flags_steering()
    flags.asByte = int("10001010",2)  # ->0010
    print(int("10001010",2))
    print( "direction: %i" % flags.direction     )
    print( "position:  %i" % flags.position )
    test = [138,35,0]
    dictionary = getAsDict(test)

    command = getAsByteArray(dictionary)
    print(command)
    if (test == command):
        print("goot")
